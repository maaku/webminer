...
{
    std::cerr << "Failed to read randomness, aborting" << std::endl;
    std::abort();
}
...
static void ReportHardwareRand()
{
    ...
    if (g_rdseed_supported) {
        std::cerr << "Using RdSeed as additional entropy source" << std::endl;
    }
    if (g_rdrand_supported) {
        std::cerr << "Using RdRand as an additional entropy source" << std::endl;
    }
}
...
static void Strengthen(const unsigned char (&seed)[32], int microseconds, CSHA512& hasher) noexcept
{
    ...
    absl::Time stop = absl::Now() + absl::Microseconds(microseconds);
    do {
...
static void SeedPeriodic(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    std::cout << "Feeding " << (hasher.Size() - old_size) << " bytes of dynamic environment data into RNG" << std::endl;
...
static void SeedStartup(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    std::cout << "Feeding " << (hasher.Size() - old_size) << " bytes of environment data into RNG" << std::endl;
